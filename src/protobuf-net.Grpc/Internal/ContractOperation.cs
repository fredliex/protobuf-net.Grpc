...
namespace ProtoBuf.Grpc.Internal
{
    internal readonly struct ContractOperation
    {
        ...
        internal static bool TryIdentifySignature(MethodInfo method, BinderConfiguration binderConfig, out ContractOperation operation, IBindContext? bindContext)
        {
            ...
            // 取得TypeCategory = Data的參數
            var dataArgsTypes = args
                .Select(arg => arg.ParameterType)
                .TakeWhile(pt => GetCategory(binderConfig.MarshallerCache, pt, bindContext) == TypeCategory.Data)
                .ToArray();
            if (args.Length - dataArgsTypes.Length > 3) return false; // too many parameters
            // 因為TypeCategory = Data的參數都會是在前面，所以替換成一個資料參數，這操作不會影響GetSignature的回傳值
            ...
            // 將資料參數型別嘗試轉為Tuple或ValueTypeWrapper
            var arg0Type =
                dataArgsTypes.Length > 1 ? GetTupledType(dataArgsTypes) :
                dataArgsTypes.Length == 1 && dataArgsTypes[0].IsValueType ? typeof(ValueTypeWrapper<>).MakeGenericType(dataArgsTypes[0]) :
                args.FirstOrDefault().ParameterType;

            // 判斷若回傳型別為值類型，轉為ValueTypeWrapper
